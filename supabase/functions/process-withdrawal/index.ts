...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "approve") {
      ...
      // If a Postgres URL is configured, fetch payout details directly from Postgres
      ...
      if (pgUrl) {
        ...
        try {
          ...
          const result = await pg.queryObject(
            `SELECT * FROM public.creator_payouts WHERE user_id = $1 LIMIT 1`,
            withdrawal.user_id,
          );
          payout = result.rows?.[0] || null;
          await pg.end();
        } catch (err) {
          console.error("Failed to query payout from Postgres, falling back to Supabase", err);
          try {
            await pg.end();
          } catch {}
          payout = null;
      ...
      if (!recipientRes.ok) {
        await supabase
          ...
          .eq("id", withdrawal_id);
        throw new Error(
          `Recipient creation failed: ${JSON.stringify(recipientData)}`,
        );
      }
      ...
      return new Response(
        JSON.stringify({ success: true, transfer: transferData.data }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
