...
import { MessageSquare, Send, Loader2, Trash2, LogIn } from "lucide-react";
...
const CommentsSection = ({ documentId, allowComments }: CommentsSectionProps) => {
  const { user } = useAuth();
        ...
        <div className="bg-card rounded-2xl border border-border p-4 flex items-center gap-4 max-w-md cursor-pointer hover:bg-muted/50 transition-colors">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              {comments.length > 0 ? `${comments.length} comments` : "Join the conversation"}
            </p>
          </div>
